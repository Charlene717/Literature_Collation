...
FolderName = c("20210410 Raw data Test") 
...
merge_1 <- read_excel(paste0(PathName,dir[1],sep=""))
...
for(i in 1:n){         #對於每個一級目錄(文件夾)
  ...
  for(j in 1:n_sub[i]){     #對於每個一級目錄(文件夾)下的每個txt文件
    ...
    new_1 <- read_excel(paste0(PathName,dir[i],sep="")) #讀取xls文件
...
## Error
# unique_Email <- unique(merge_1$`Email Addresses`)
# unique_Email <- unique(merge_1)
# merge_1 <- merge_1[merge_1$`Email Addresses` %in% unique_Email,]
# merge_1 <- merge_1[merge_1$`Email Addresses` %in% unique_Email,]
...
write.table(unique_Email2,paste(PathName,'/',FolderName,'_merge.txt',sep=''),row.names = F,col.names= F)#單獨保存在每個文件夾下
write.table(unique_Email2,paste(PathName,'/',FolderName,'_merge.csv',sep=''),row.names = FALSE,col.names= FALSE)#單獨保存在每個文件夾下
